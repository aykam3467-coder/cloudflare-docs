...
export type Provider = "openai" | "anthropic" | "google" | "grok" | "dynamic";
...
export function useAIGConfig() {
    const [config, setConfig] = useState<Config>({ provider: "openai", keyType: "byok", clientType: "openai-js", apiType: "unified" });
...
type ProviderItem = { value: Provider; label: string, icon?: string, invertIconDark?: boolean }
...
const renderOptionWithIcon = (option: ProviderItem) => {
    return {
        label: <div className="flex items-center gap-3"><img className={option.invertIconDark ? "dark:invert" : ""} src={`data:image/svg+xml;base64,${option.icon}`} style={{ width: "12px", height: "12px" }} /> {option.label}</div>,
        value: option.value
    };
};
...
export function CodeSelector({ forceClient, forceApiType }: { forceClient?: ClientType; forceApiType?: APIType }) {
    const { config, updateConfig } = useAIGConfig();

    const provider = providerOptions.find(p => p.value === config.provider) || providerOptions[0];
    const clientType = clientTypeOptions.find(c => c.value === (forceClient ?? config.clientType)) || clientTypeOptions[0];
    const keyType = keyTypeOptions.find(k => k.value === config.keyType) || keyTypeOptions[0];
    const apiType = apiTypeOptions.find(a => a.value === (forceApiType ?? config.apiType)) || apiTypeOptions[0];

    return <div className="not-content px-4 mt-2 rounded-lg">
        <div className="flex flex-col">
            <div className="flex items-center gap-2 flex-wrap">Make a request to <ReactSelect
                // className="mt-2 h-16"
                ...
                styles={{
                    menu: (base) => ({ ...base, zIndex: 999, minWidth: "12rem", backgroundColor: "var(--selector-bg-color)" }),
                    ...BaseReactSelectStyles,
                }}
                placeholder="Select an AI provider"
            />{!forceApiType && <><ReactSelect
                value={apiType}
                ...
                styles={{
                    menu: (base) => ({ ...base, zIndex: 999, minWidth: "10rem", backgroundColor: "var(--selector-bg-color)" }),
                    ...BaseReactSelectStyles,
                }}
                placeholder="Select API type"
            />API </>}{!forceClient && <>using <ReactSelect
                value={clientType}
            ...
            /><div className="w-full" /></>}{config.provider !== "dynamic" && <>with <ReactSelect
                // className="mt-2 h-16"
                value={keyType}
...
export function CodeExample({ provider, clientType, keyType, apiType, forceApiType, forceClientType, children }: { provider: string; clientType: string; keyType: string; apiType: string; forceApiType?: string; forceClientType?: string; children: React.ReactNode }) {
    const { config } = useAIGConfig();
    if (config.provider === "dynamic" && config.keyType === "in-request") {
        config.keyType = "byok";
    }
    ...
    const isHidden = config.provider !== provider
...
export const modelOptions = [
    { model: "gpt-5.2", provider: "openai", aiSDK: { providerFactory: "createOpenAI", providerUsage: "openai.chat", provider: "openai" } },
    { model: "claude-4-5-sonnet", provider: "anthropic", aiSDK: { providerFactory: "createAnthropic", providerUsage: "anthropic", provider: "anthropic" } },
    { model: "gemini-2.5-pro", provider: "google", aiSDK: { providerFactory: "createGoogle", providerUsage: "google", provider: "google" } },
    { model: 'grok-4', provider: 'grok', aiSDK: { providerFactory: "createXai", providerUsage: "xai", provider: "xai" } },
    { model: 'customer-support', provider: 'dynamic', aiSDK: { providerFactory: "createUnified", providerUsage: "unified", provider: "unified" } }
];
